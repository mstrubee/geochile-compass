...
/** Escala quintil → color verde-amarillo-rojo (1=bajo, 5=alto potencial) */
...
export const AgroplanetComunasLayer = ({ visible, scoreMode }: Props) => {
  ...
  // Re-estilizar cuando cambia el modo o los datos
  useEffect(() => {
  ...
  }, [scoreMode, agroData, loading]);

  if (!visible || !ready || !fc) return null;

  const getScore = (cut: string): number => {
  ...
  const onEachFeature = (feature: Feature<Geometry, ComunaProps>, layer: Layer) => {
    ...
    layer.on({
      mouseover(e) {
        ...
        if (prev && prev !== layer) {
          geoJsonRef.current?.resetStyle(prev);
        }
